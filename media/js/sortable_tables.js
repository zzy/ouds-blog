/* 
thinkerunion.net by ÕÅæðÖ® (ouds@thinkerunion.net)
Copyright (c) 2007 thinkerunion.net. Please don't steal.

On page load, the SortableManager:

- Finds the table by its id (sortable_table).
- Parses its thead for columns with a "tu:formatt" attribute.
- Parses the data out of the tbody based upon information given in the
  "tu:formatt" attribute, and clones the tr elements for later re-use.
...
update(SortableManager.prototype, {
    ...
    "initWithTable": function (table) {
        ...
        ***/
        ...
        // get the tu:formatt key and contents for each column header
        var cols = this.thead.getElementsByTagName('th');
        for (var i = 0; i < cols.length; i++) {
            var node = cols[i];
            var attr = null;
            try {
                attr = node.getAttribute("tu:formatt");
            } catch (err) {
                // pass
            }
